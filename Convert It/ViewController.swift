...
class ViewController: UIViewController {
   ...
   let formulasArray = [Formula(conversionString: "miles to kilometers", formula: {$0 / 0.62137}),
                        ...
                        Formula(conversionString: "fahrenheit to celsius", formula: {($0 - 32) * (5.0/9.0)}),
                        Formula(conversionString: "celsius to fahrenheit", formula: {($0 * (9.0/5.0)) + 32}),
    ...
    func calculateConversion(){
        guard let inputValue = Double(userInput.text!) else{
            if userInput.text != ""{
                showAlert(title: "Cannot Convert Value", message: "\"\(String(describing: userInput.text))\" is not a valid number.")
            }
            return
        }
    ...
    @IBAction func decimalSegment(_ sender: UISegmentedControl) {
        calculateConversion()
    }
